...
@TeleOp(name = "HereWeGoRed", group = "Sensor")
//@Disabled
public class HereWeGoRed extends LinearOpMode {
    ...
    public void turnAbsolute(int target) {
        int heading = cleanUp(180 - mrGyro.getHeading()); //Set variable to gyro readings
        for (int i = 0; i < 2; i++) {
            while (Math.abs(heading - target) > 180) {  //Continue while the robot direction is further than 180 degrees from the target
                ...
                heading = cleanUp(180 - mrGyro.getHeading());  //Set variable to gyro reading
    ...
    public void gyroRead() {
        heading = 180 - mrGyro.getHeading();  // Reverse direction of heading to match the integrated value
    ...
    public void findHeading() {
        ...
        heading = cleanUp(180 - mrGyro.getHeading());  // Reverse direction of heading to match the integrated value.
        temp1 = heading;
        if (temp > temp1)
            temp1 += 360;
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            telemetry.addData("Heading", String.format("%03d", heading));  // Display variables to Driver Station Screen
            telemetry.addData("Target", String.format("%03d", target));

            telemetry.addData("ODS Raw", odsReadingRaw);

            telemetry.addData("Ultra Sonic", range1Cache[0] & 0xFF);
            telemetry.addData("Range ODS", range1Cache[1] & 0xFF);
